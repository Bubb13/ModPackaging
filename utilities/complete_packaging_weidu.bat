...
REM set sfx_ico=%ico_folder%\g3icon.ico
set sfx_banner=%ico_folder%\weidu_banner.bmp
set sfx_conf=mod.conf

REM /* generate HTML versions of Markdown files */
if [%generate_readme%]==[1] (
...
REM SET folder_icon=%ico_folder%\g3.ico
...
if [%build_windows%]==[1] (
  ...
  copy /y "%~dp0\weidu.org.conf" "%sfx_conf%" >nul
  ...
  "%~dp0\winrar.exe" a -sfx -z%sfx_conf% -iimg"%sfx_banner%" -x%sox% -x%tisunpack_unix% -x%tisunpack_osx% -x%tile2ee_unix% -x%tile2ee_osx% %win_archive% %win_files%
...
if [%build_osx%]==[1] (
  ...
  "%~dp0\7za.exe" a -x^!%oggdec% -x^!%tisunpack_win32% -x^!%tisunpack_unix% -x^!%tile2ee_win32% -x^!%tile2ee_unix% -x^!%iconv% -x^!%desktop_ini% -x^!%sfx_banner% %osx_archive% %osx_files% >nul
...
if [%build_linux%]==[1] (
  ...
  "%~dp0\7za.exe" a -x^!%oggdec% -x^!%sox% -x^!%tisunpack_win32% -x^!%tisunpack_osx% -x^!%tile2ee_win32% -x^!%tile2ee_osx% -x^!%iconv% -x^!%desktop_ini% -x^!%sfx_banner% %lin_archive% %lin_files% >nul
